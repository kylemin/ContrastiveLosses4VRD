...
if __name__ == '__main__':
    ...
    with open(os.path.join('data', args.dataset,'objects.json'), 'r') as f:
        obj_list = json.load(f)
        f.close()
    with open(os.path.join('data', args.dataset, 'predicates.json'), 'r') as f:
        rel_list = json.load(f)
        f.close()
    
    print('Loading test_videos_list.json')
    if args.dataset.find('ag') >= 0:
        val_map_list_path = os.path.join('data', args.dataset, 'test_videos_list.json')
